...
class PostUpdatePreview extends React.Component {
  render() {
    let getTranslation = (key) => {
      return 'Agenda.publish.modal.' + key;
    };
    const { formName } = this.props;
    let form = this.props[formName];
    if (!form) {
      return null;
    }
    let post = {
...
export default connect(state => {
  return {
    postCreate: state.form['post-create'] ? state.form['post-create'].values : null,
    postUpdate: state.form['post-update'] ? state.form['post-update'].values : null
  };
})(PostUpdatePreview);
